...
with open("../cache/pdb.txt", "r") as pdb:
    ...
    for line in f_strip:
        ...
        phrase_fix[phrase] = phrase_pinyin

pypinyin.load_phrases_dict(phrase_fix)
...
class DictGenerator:
    ...
    def getWordDictText(self):
        ...
        for word_st in word_list:
            if not word_st[2] == 1:
                text_dict += (
                    word_st[0] + "\t" + word_st[1] + "\t" + str(word_st[2]) + "\n"
                )
        return text_dict
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Glime raw pinyin dict generator.")
    parser.add_argument(
        "--minfreq",
        "-m",
